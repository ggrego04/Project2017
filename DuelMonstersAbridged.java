...
/**
 * Represents the tester/runner file for the Duel Monster Abridged game.
 *
 */
public class DuelMonstersAbridged {
    ...
    private static void playerPlayCard() {
        ...
        if (selection >= 0 && selection < player.getHand().length
            && player.getHand()[selection] != null) {
    ...
    private static void playerAttackCard() {
        ...
        if (myMonster >= 0 && enemyMonster >= 0
            && myMonster < playerField.getMonsters().length
            && enemyMonster < cpuField.getMonsters().length
            && playerField.getMonsters()[myMonster] != null
            && playerField.getMonsters()[myMonster].getCanAttack()
            && cpuField.getMonsters()[enemyMonster] != null) {
            success = playerField.getMonsters()[myMonster].getPower()
                >= cpuField.getMonsters()[enemyMonster].getPower();
            if (success) {
                cpu.changeLifePoints(
                    cpuField.getMonsters()[enemyMonster].getPower()
                    - playerField.getMonsters()[myMonster].getPower());
                cpuField.getMonsters()[enemyMonster] = null;
                playerField.getMonsters()[myMonster].setCanAttack(false);
            }
        }
        status = (success
            ? "Attack successful!"
    ...
    private static void printGameState() {
        ...
        System.out.println();
        printCards(spellCardStrings, cpuField.getSpells());
        printCards(monsterCardStrings, cpuField.getMonsters());
        System.out.println();
        System.out.println("Enemy Monsters");
        System.out.println();
        printNumberedArray(cpuField.getMonsters());
        System.out.println("Enemy Spells");
        printNumberedArray(cpuField.getSpells());
        System.out.println();
        ...
        if (player.getNextSpecial() != null) {
            System.out.println(playerField.getMonsters().length + 1 + ": "
     ...
     */
    private static void printNumberedArray(Object[] things) {
        int i = 1;
        for (Object c : things) {
            if (c != null) {
    ...
    private static void setupPlayers() {
        ...
        SpecialDeck playerSpecials = new SpecialDeck(
                new DestroySpell(),
                new BlueEyesWhiteDragon()
                );
        SpecialDeck cpuSpecials = new SpecialDeck(
                new DestroySpell(),
                new BlueEyesWhiteDragon()
                );
